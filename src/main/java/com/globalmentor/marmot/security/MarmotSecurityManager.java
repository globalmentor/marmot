...
public interface MarmotSecurityManager {
	 ...
	 * @throws ResourceIOException if there is an error accessing the repository.
	 */
	public boolean isAllowed(final Principal owner, final Repository repository, final URI resourceURI, final Principal user, final boolean checkAncestors,
			final PermissionType... permissionTypes) throws ResourceIOException;

	/**
	 * Determines whether a given user has at least one permission to perform some action in relation to a given repository and resource. This method is additive;
	...
	/**
	 * Determines whether a given user has some permission to perform some action in relation to a given repository and resource. This method is additive; if a
	 * superclass doesn't find a permission, a subclass may be able to add the permission.
	 * @param owner The principal that owns the repository.
	 * @param repository The repository that contains the resource.
	 * @param user The user attempting to access the resource, which may be <code>null</code> if the user is anonymous.
	...
	/**
	 * Determines whether a given user has permission to perform some action in relation to a given repository and resource. This method is additive; if a
	 * superclass doesn't find a permission, a subclass may be able to add the permission. This implementation allows all permissions if the user is the owner.
	 * @param owner The principal that owns the repository.
	 * @param repository The repository that contains the resource.
	 * @param user The user attempting to access the resource, which may be <code>null</code> if the user is anonymous.
