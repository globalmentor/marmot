...
public class RepositorySynchronizer
{
	...
	public enum Resolution
	{
		...
		CONSUME,
		/**
		 * Newer information will overwrite older information; the source and destination are intended to be updated with the latest changes from each, although for
		 * orphan resources this will be considered the same as {@link #BACKUP}.
		 */
		SYNCHRONIZE,
		/** Nothing will happen. */
		IGNORE;
	...
	/**
	 * Convenience method that sets the resolution at the resource, content, and metadata level.
	...
	/**
	 * Synchronizes two resources in two separate repositories. If the resources are collections, the child resources will also be synchronized.
			...
			final URI destinationResourceURI) throws IOException
	{
		...
		final URFResource sourceResourceDescription = sourceRepository.resourceExists(sourceResourceURI) ? sourceRepository
				.getResourceDescription(sourceResourceURI) : null; //get the description of the source resource if it exists
		final URFResource destinationResourceDescription = destinationRepository.resourceExists(destinationResourceURI) ? destinationRepository
				.getResourceDescription(destinationResourceURI) : null; //get the description of the destination resource if it exists
		synchronize(sourceRepository, sourceResourceURI, sourceResourceURI, sourceResourceDescription, destinationRepository, destinationResourceURI,
				destinationResourceURI, destinationResourceDescription); //synchronize using the descriptions and the initial URIs as the base URIs
		Log.info(getTestStatus(), "Synchronization finished.");
	}

	/**
	 * Synchronizes two resources in two separate repositories. If the resources are collections, the child resources will also be synchronized. If either
	 * resource is one to be ignored, no action is taken.
	 * @param sourceRepository The repository in which the source resource lies.
	...
	/**
	 * Checks to see if the content of two existing resources are mirrors of one another. This method attempts to guess whether content has changed by examining
	 * various properties. Two collections with the same relevant individual properties are considered mirrors; child resources are not examined.
	 * @param sourceRepository The repository in which the source resource lies.
	...
	/**
	 * Resolves a content discrepancy between a source and a destination resource. If the resource dates are the same, the {@link Resolution#SYNCHRONIZE}
	 * resolution will have no effect. If only one of the resource dates is available, it is considered newer for the purpose of the
	 * {@link Resolution#SYNCHRONIZE} resolution. If neither resource date is available, the {@link Resolution#SYNCHRONIZE} resolution will be considered the same
	 * as the {@link Resolution#BACKUP} resolution. Both resources must exist.
	...
	/**
	 * Resolves metadata discrepancies between a source and a destination resource. If the resource dates are the same, the {@link Resolution#SYNCHRONIZE}
	 * resolution will be considered the same as the {@link Resolution#BACKUP} resolution. If only one of the resource dates is available, it is considered newer
	 * for the purpose of the {@link Resolution#SYNCHRONIZE} resolution. If neither resource date is available, the {@link Resolution#SYNCHRONIZE} resolution will
	 * be considered the same as the {@link Resolution#BACKUP} resolution. The content modified date is ignored for zero-length collections for determining the
	 * correct {@link Resolution#SYNCHRONIZE} resolution. If the resource dates are not available or are the same, the {@link Resolution#SYNCHRONIZE} resolution
	 * will have no effect. This method ignores the {@link Content#MODIFIED_PROPERTY_URI} property. Both resources must exist.
	 * @param resolution How the discrepancy should be resolved
	 * @param sourceRepository The source repository.
	 * @param sourceResourceDescription The description of the source resource.
	 * @param sourceContentModified The date and time at which the content of the source was modified, or <code>null</code> if not known.
