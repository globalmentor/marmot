...
public class MarmotMirror extends AbstractApplication
{
	...
	public enum Parameter implements Identifier
	{
		...
		/** Whether metadata should be unconditionally updated for the determined output resource. */
		FORCE_WRITE_METADATA,
		/** Whether metadata should be unconditionally rewritten for the source resource. */
		REWRITE_SOURCE_METADATA,
		/** Whether metadata should be unconditionally rewritten for the destination resource. */
		REWRITE_DESTINATION_METADATA,
		/** Test mode. */
		TEST,
		/** Verbose output. */
		VERBOSE,
	...
	public int main()
	{
		...
		if(sourceRepositoryString == null || destinationRepositoryString == null) //if the source and/or destination repository parameter is missing
		{
			...
			System.out.println("Usage: MarmotMirror --source-repository=<file|URI> [--source-repositorytype=<repository type>] [--source-username=<username>] [--source-password=<password>] [--source-resource=<file|URI>] " +
					...
					"[--resolution=<resolution>] [--resource-resolution=<resolution>] [--content-resolution=<resolution>] [--metadata-resolution=<resolution>]" +
					"[--force-content-modified-property] [--force-write-metadata] [--rewrite-source-metadata] [--rewrite-destination-metadata] [--test] [--verbose] [--debug-http]");
			System.out.println("");
			...
			System.out.println(LONG_SWITCH_DELIMITER+getSerializationName(Parameter.FORCE_WRITE_METADATA)+": Whether metadata should be unconditionally rewritten for the source resource.");
			System.out.println(LONG_SWITCH_DELIMITER+getSerializationName(Parameter.REWRITE_SOURCE_METADATA)+": Whether metadata should be unconditionally rewritten for the destination resource.");
			System.out.println(LONG_SWITCH_DELIMITER+getSerializationName(Parameter.REWRITE_DESTINATION_METADATA)+": Whether metadata should be unconditionally updated for the determined output resource.");
		...
		final Repository sourceRepository = createRepository(
				sourceRepositoryTypeString != null ? getSerializedEnum(RepositoryType.class, sourceRepositoryTypeString) : null, sourceRepositoryURI,
				getOption(args, Parameter.SOURCE_USERNAME), getOption(args, Parameter.SOURCE_PASSWORD)); //create the correct type of repository for the source
	 ...
	 * <p>
	 * If no repository type is specified, a type is guessed. The following repository schemes are recognized:
	 * </p>
	 * <dl>
	 * <dt>{@value HTTP#HTTP_URI_SCHEME}</dt>
	 * <dd>{@link RepositoryType#WEBDAV}</dd>
	 * <dt>{@value HTTP#HTTPS_URI_SCHEME}</dt>
	 * <dd>{@link RepositoryType#WEBDAV}</dd>
	 * <dt>{@value URIs#FILE_SCHEME}</dt>
	 * <dd>{@link RepositoryType#FILE}</dd>
	 * </dl>
